...
// the text input where the user should type their name
...
function validate(event) {
    ...
    var searchIsValid = username.length > 0;
    if (searchIsValid == false) {
        giveFeedback(searchContainer, "You must tell me your name!");
    }

    if (usernameIsValid == true && searchIsValid == true) {
...
function invalidChars(str, availableChars) {
    ...
    for (x = 0; x < str.length; x++) {
        for (y = 0; y < availableChars.length; y++) {
            if (str[x] != availableChars[y]) {
                invalids.push(str[x]);
            }
        }
    }    
...
function giveFeedback(container, message) {
    ...
    feedbackLabel.innerHTML = "Test", message + invalids;
}

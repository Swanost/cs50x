...
char* indexes(const char* path)
{
    ...
    char* newhtml = malloc(strlen(html) + strlen(path) + 1);
    strcpy(newhtml, path);
    strcat(newhtml, html);
    
    char* newphp = malloc(strlen(php) + strlen(path) + 1);
    strcpy(newphp, path);
    strcat(newphp, php);
    
    if(access(newhtml, F_OK) == 1)
    {
        return newhtml;
    }
    
    if(access(newphp, F_OK) == 1)
    {
        return newphp;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // null content and zero out length
    *content = NULL;
    *length = 0;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // Ran into problems if I didn't clear out the data
    // Started with '0' but even that was adding onto the URL
    // Don't know how to fix otherwise
    
    for (int m = 0; m < strlen(target); m++)
        {
            abs_path[m] = ' ';
        }
